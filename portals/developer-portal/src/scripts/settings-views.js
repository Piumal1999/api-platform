...
(function () {
  var views = [];
  try { views = JSON.parse(document.getElementById('dp-settings__views-data').textContent) || []; } catch (e) { /* ignore */ }
  var editHandle = null;
  var modal = document.getElementById('dp-settings__view-modal');
  if (!modal) return;
  ...
  if (labelPicker) {
    labelPicker.addEventListener('click', function (e) {
      var chip = e.target.closest('.dp-settings__label-toggle');
      if (!chip) return;
  ...
  function setSelectedLabels(labels) {
    ...
    labelPicker.querySelectorAll('.dp-settings__label-toggle').forEach(function (chip) {
  ...
  function getSelectedLabels() {
    ...
    return Array.prototype.map.call(
      labelPicker.querySelectorAll('.dp-settings__label-toggle[aria-pressed="true"]'),
  ...
  function openModal(mode, view) {
    ...
    g('dp-settings__view-modal-title').textContent = mode === 'edit' ? 'Edit view' : 'Add view';
    g('dp-settings__view-modal-save').textContent  = mode === 'edit' ? 'Save changes' : 'Add view';
  ...
  var addBtn = g('dp-settings__add-view-btn');
  if (addBtn) addBtn.addEventListener('click', function () { openModal('add'); });
  g('dp-settings__view-modal-close').addEventListener('click', closeModal);
  g('dp-settings__view-modal-cancel').addEventListener('click', closeModal);
  ...
  g('dp-settings__view-modal-save').addEventListener('click', async function () {
  ...
  document.addEventListener('click', function (e) {
    var editBtn = e.target.closest('.dp-settings__view-edit-btn');
    if (editBtn) {
      var view = views.filter(function (vw) { return vw.id === editBtn.dataset.id; })[0];
      if (view) openModal('edit', view);
      return;
    }
    var delBtn = e.target.closest('.dp-settings__view-delete-btn');
    if (delBtn) {
      pendingDel = delBtn.dataset.id;
      g('dp-settings__del-view-name-txt').textContent = delBtn.dataset.name || delBtn.dataset.id;
      g('dp-settings__delete-view-modal').style.display = 'flex';
      return;
    }
  });
  g('dp-settings__del-view-cancel').addEventListener('click', function () { g('dp-settings__delete-view-modal').style.display = 'none'; });
  g('dp-settings__delete-view-modal').addEventListener('click', function (e) { if (e.target === this) this.style.display = 'none'; });
  g('dp-settings__del-view-confirm').addEventListener('click', async function () {
    if (!pendingDel) return;
    g('dp-settings__delete-view-modal').style.display = 'none';
    try {
